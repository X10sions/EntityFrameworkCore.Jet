...
namespace EntityFrameworkCore.Jet.Metadata.Internal {
  ...
  public class JetAnnotationProvider : RelationalAnnotationProvider {
    ...
    public override IEnumerable<IAnnotation> For(ITableIndex index, bool designTime) {
      ...
      IIndex modelIndex = index.MappedIndexes.First();
    ...
    public override IEnumerable<IAnnotation> For(IColumn column, bool designTime) {
      ...
      IProperty? property = column.PropertyMappings.Where(m => (m.TableMapping.IsSharedTablePrincipal ?? false) && m.TableMapping.EntityType == m.Property.DeclaringEntityType)
    .Select(m => m.Property)
    .FirstOrDefault(
        p => p.GetValueGenerationStrategy(table)
            == JetValueGenerationStrategy.IdentityColumn);
      if (property != null) {
        int? seed = property.GetIdentitySeed(table);
        int? increment = property.GetIdentityIncrement(table);
